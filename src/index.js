...
function sourceless(options) {
  var scope = options.scope;
  var targetWindow = options.window;
  var eventName = "sourcelessMessage";
  var windowForEventListening = options.windowForEventListening || window;
  var listeners = {};
  var sendBuffer = [];
  var listenBuffer = {};
  ...
  windowForEventListening.addEventListener(eventName, listener, false);
  ...
  var postis = {
    ...
    send: function (opts) {
      ...
      if ((ready || opts.method === readyMethod)) {
        var event = new CustomEvent(eventName, { detail: { scope: scope, method: method, params: opts.params } });
    ...
    destroy: function (callback) {
      ...
      ready = false;
      if (windowForEventListening && typeof windowForEventListening.removeEventListener === "function") {
        windowForEventListening.removeEventListener(eventName, listener);
      }
      callback && callback();
    }
  };
...
function Postis(options) {
  if (options.type && options.type === "sourceless") {
    return sourceless(options);
  } else {
    return sourced(options);
  }
}
